  ...
  try {
    ...
    if (event.Records && event.Records[0].EventSource === "aws:sns") {
      ...
      try {
        ...
        snsMessage = { message: event.Records[0].Sns.Message };
      }
...
function formatContactMessage(body) {
  // ✅ Fixed — body is already a parsed object
...
function formatCloudWatchMessage(detail) {
  const color = detail.errorMessage ? "#ff0000" : "#36a64f";
  const title = detail.errorMessage ? "❌ Error Detected" : "📊 CloudWatch Event";
